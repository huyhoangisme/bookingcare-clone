...
class ManageClinic extends Component {
    ...
    async componentDidUpdate(prevProps, prevState) {
        if (prevProps.clinics !== this.props.clinics) {
            ...
            this.setState({
                options: this.buildChooseSelectSpeciality(this.state.clinics),
            });
        }
    }
    ...
    handleChangeSelect = async (selectedOption) => {
        let item = await handleGetClinic(selectedOption.value);
        let imageBase64 = "";
        if (item.data.image) {
    ...
    handleClickBtnSave = async () => {
        ...
        await handleUpdateClinic(selectedOption.value, {
